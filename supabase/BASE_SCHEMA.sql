  ...
  alter table public.profiles drop constraint if exists profiles_points_balance_check;
  alter table public.profiles add constraint profiles_points_balance_check check (points_balance >= 0);
  ...
  create table if not exists public.books (
    ...
    access_points integer not null default 0,
    page_count integer check (page_count is null or page_count > 0),
    created_at timestamptz not null default now(),
    check (available_copies <= total_copies)
  );

  alter table public.books add column if not exists access_points integer not null default 0;
  alter table public.books add column if not exists page_count integer;
  alter table public.books drop constraint if exists books_page_count_check;
  alter table public.books add constraint books_page_count_check check (page_count is null or page_count > 0);
  alter table public.books drop constraint if exists books_access_points_check;
  alter table public.books add constraint books_access_points_check check (access_points >= 0);

  create table if not exists public.borrow_requests (
    id bigint generated by default as identity primary key,
    student_id uuid not null references public.profiles(id) on delete cascade,
    book_id bigint not null references public.books(id) on delete cascade,
    status text not null default 'pending' check (status in ('pending', 'approved', 'rejected', 'returned', 'revoked')),
  ...
  alter table public.borrow_requests
    ...
    check (status in ('pending', 'approved', 'rejected', 'returned', 'revoked'));
  alter table public.borrow_requests add column if not exists points_cost integer not null default 0;
  alter table public.borrow_requests drop constraint if exists borrow_requests_points_cost_check;
  alter table public.borrow_requests add constraint borrow_requests_points_cost_check check (points_cost >= 0);

  create table if not exists public.point_transactions (
    id bigint generated by default as identity primary key,
    user_id uuid not null references public.profiles(id) on delete cascade,
    amount integer not null check (amount <> 0),
    transaction_type text not null check (transaction_type in ('contribution_reward', 'access_unlock', 'admin_adjustment')),
    reference_id bigint,
  ...
  create table if not exists public.study_drawings (
    id bigint generated by default as identity primary key,
    user_id uuid not null references public.profiles(id) on delete cascade,
    book_id bigint not null references public.books(id) on delete cascade,
  ...
  create index if not exists idx_book_contributions_status on public.book_contributions(status, created_at desc);
  create index if not exists idx_study_drawings_user_book on public.study_drawings(user_id, book_id, page);
  ...
  begin
    if old.status is distinct from new.status and new.status in ('approved', 'rejected') then
      ...
      values (
        ...
        case when new.status = 'approved' then 'Access approved' else 'Request update' end,
        case when new.status = 'approved' then 'Your digital library request was approved.' else 'Your digital library request was not approved.' end,
        new.id
      );
    end if;
    return new;
  ...
  create or replace function public.submit_book_contribution(
    p_title text,
    p_author text,
    p_description text default null,
    p_pdf_url text default null
  )
  returns public.book_contributions language plpgsql security definer set search_path = public as $$
  declare
  ...
  begin
    ...
    insert into public.book_contributions(user_id, title, author, description, pdf_url)
    values (auth.uid(), trim(p_title), trim(p_author), nullif(trim(p_description), ''), nullif(trim(p_pdf_url), ''))
  ...
  begin
    ...
    if decision = 'approved' then
      insert into public.books(title, author, category, description, pdf_url, access_points)
      values (contribution.title, contribution.author, 'Community contribution', contribution.description, contribution.pdf_url, access_points)
    ...
    set status = decision,
        points_awarded = case when decision = 'approved' then reward_points else 0 end,
        approved_book_id = created_book_id,
  ...
  create or replace function public.revoke_digital_loan(request_id bigint)
  returns public.borrow_requests language plpgsql security definer set search_path = public as $$
  declare
    updated_request public.borrow_requests;
  begin
    if not public.is_admin() then raise exception 'Administrator access required'; end if;
    update public.borrow_requests
    set status = 'revoked', returned_date = now()
    where id = request_id and status = 'approved' and returned_date is null
    returning * into updated_request;
    if updated_request.id is null then raise exception 'Approved loan not found or already closed'; end if;
    return updated_request;
  end;
  $$;
  ...
  alter table public.study_drawings enable row level security;
  ...
  drop policy if exists study_drawings_manage_own_or_admin on public.study_drawings;
  create policy study_drawings_manage_own_or_admin on public.study_drawings
  for all to authenticated
  using (user_id = auth.uid() or public.is_admin())
  with check (user_id = auth.uid() or public.is_admin());
  ...
  drop policy if exists point_transactions_read_own_or_admin on public.point_transactions;
  create policy point_transactions_read_own_or_admin on public.point_transactions
  for select to authenticated using (user_id = auth.uid() or public.is_admin());

  drop policy if exists book_contributions_read_own_or_admin on public.book_contributions;
  create policy book_contributions_read_own_or_admin on public.book_contributions
  for select to authenticated using (user_id = auth.uid() or public.is_admin());

  drop policy if exists book_contributions_insert_own on public.book_contributions;
  create policy book_contributions_insert_own on public.book_contributions
  ...
  drop policy if exists reading_progress_manage_own on public.reading_progress;
  create policy reading_progress_manage_own on public.reading_progress
  for all to authenticated using (user_id = auth.uid()) with check (user_id = auth.uid());
  ...
  grant select, insert, update, delete on public.study_drawings to authenticated;
  grant select, update on public.fines to authenticated;
  grant select on public.point_transactions to authenticated;
  grant select, insert on public.book_contributions to authenticated;
  ...
  grant execute on function public.submit_book_contribution(text, text, text, text) to authenticated;
  ...
  grant execute on function public.revoke_digital_loan(bigint) to authenticated;
